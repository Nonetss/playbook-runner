...
## Purpose
Provide runtime locale switching for the frontend so users see the UI in their preferred language (es + en at launch) without changing URLs, with locale resolved consistently between the server render and the client hydration so there is no flash of the wrong language.
